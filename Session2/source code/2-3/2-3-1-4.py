...
# open 함수를 통해 'data.txt' 파일을 열고 그 내용을 가져온다
with open('data.txt', 'r', encoding = 'utf-8') as f:			# 읽기 형식('r')로 지정하고 인코딩은 'utf-8'으로 설정한다
	words = f.readlines()											# readlines 함수를 통해 전체 텍스트 파일의 내용을 리스트로 가져온다 (줄로 구분)
	f.close()														# 파일을 닫는다

# open 함수를 통해 'result-2-3-1-4.txt' 파일을 열고 이를 f로 지정한다
with open('result-2-3-1-4.txt', 'w', encoding = 'utf-8') as f:		# 쓰기 형식('w')로 지정하고 인코딩은 'utf-8'로 설정한다 
